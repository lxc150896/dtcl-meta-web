...
export default function TechnologiesPage() {
  ...
  console.log("TechnologiesPage data:", data.technologies);
      ...
      <div className="overflow-x-auto mt-1 md:mt-2">
        <div className="divide-y divide-[#222]">
          <div className="grid grid-cols-[5%_40%_55%] md:grid-cols-[5%_40%_55%] bg-gray-800 text-xs md:text-sm font-bold text-gray-300 py-2 text-center">
            <div>#</div>
            <div className="text-left px-2">Lõi nâng cấp</div>
            <div className="text-left px-2">Chi tiết</div>
          </div>
          ...
          {items.map((item, index: number) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 px-2">
                <TechnologieImage techImg={item.image} baseUrl={data.base_url} />
                <span className="font-semibold truncate text-xs md:text-sm">{item.name}</span>
              </div>
              <div className="px-2 text-gray-300 text-xs md:text-sm font-medium">
                {item.description}
              </div>
            </div>
          ))}
