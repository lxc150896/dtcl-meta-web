...
import { ChevronDown, House, Search, Globe } from 'lucide-react';
import Image from 'next/image';

export default function HeaderMenu() {
  const pathname = usePathname();
  const router = useRouter();
  const { data, language, setLanguage } = useData();
  ...
  const LANGUAGE_OPTIONS = [
    ...
    { value: 'ja' as const, label: t.language.ja },
  ];

  // Map TABS với translations
  const getTabsWithTranslations = () => {
    return TABS.map((tab, index) => {
      let translatedLabel = tab.label;
      switch (tab.path) {
        case '/':
          translatedLabel = t.tabs.home;
          break;
        case '/rankings':
          translatedLabel = t.tabs.rankings;
          break;
        case '/champions':
          translatedLabel = t.tabs.champions;
          break;
        case '/items':
          translatedLabel = t.tabs.items;
          break;
        case '/synergies':
          translatedLabel = t.tabs.synergies;
          break;
        case '/augments':
          translatedLabel = t.tabs.augments;
          break;
        case '/instruct':
          translatedLabel = t.tabs.instruct;
          break;
        default:
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      const clickedOutsideDesktop = langDropdownRefDesktop.current && !langDropdownRefDesktop.current.contains(event.target as Node);
      const clickedOutsideMobile = langDropdownRefMobile.current && !langDropdownRefMobile.current.contains(event.target as Node);
  ...
  const renderLanguageDropdown = () => (
    <div className="absolute top-full right-0 mt-1 bg-white border border-gray-300 rounded shadow w-40 max-h-60 overflow-y-auto text-black z-50">
      {LANGUAGE_OPTIONS.map((lang) => (
        <div
          ...
          className={`px-3 py-2 text-sm hover:bg-gray-100 cursor-pointer ${
            language === lang.value ? 'bg-gray-200 font-semibold' : ''
          }`}
        >
          {lang.label}
        </div>
    ...
    <header className="text-white w-full sticky top-0 z-50">
        ...
        <div className="relative flex justify-center w-full">
          ...
          <div className="w-full max-w-[1080px] flex flex-col justify-between items-center gap-x-4 flex-wrap pt-4 px-4">
            ...
            <div className="grid grid-cols-[1fr_auto] w-full mb-1 items-center gap-4">
              ...
              {/* Ảnh tải app và Language Selector */}
              <div className="flex items-center gap-4">
                {/* Language Selector */}
                <div className="relative" ref={langDropdownRefDesktop}>
                  <div
                    className="px-3 py-2 bg-gray-700 text-white text-xs cursor-pointer flex items-center gap-1 rounded border border-gray-600 hover:bg-gray-600"
                  ...
                  {isLangDropdownOpen && renderLanguageDropdown()}
                </div>
            ...
            <div className="w-full flex-1 max-w-[1080px] mx-auto py-1 overflow-x-auto">
              <div className="flex items-center gap-2 w-max">
                ...
                {getTabsWithTranslations().map((tab, i) => {
          ...
          {/* === Cột phải: QR / Version === */}
            <div className="hidden lg:flex flex-col items-center text-center absolute right-4 top-4">
              ...
              <div className="text-[10px] text-[#ffb900] mt-1">
                {t.common.version}: {data?.version?.season_meta}
              </div>
            </div>
        </div>
      </div>
      ...
      <div className="md:hidden border-b border-gray-700 px-4 py-2 bg-black">
        <div className="flex items-center justify-between gap-2">
          ...
          {/* Language Selector và Toggle menu button */}
          <div className="flex items-center gap-2">
            {/* Language Selector Mobile */}
            <div className="relative" ref={langDropdownRefMobile}>
              <div
                className="px-2 py-1.5 bg-gray-700 text-white text-xs cursor-pointer flex items-center gap-1 rounded border border-gray-600"
                onClick={() => setIsLangDropdownOpen(!isLangDropdownOpen)}
              >
              ...
              {isLangDropdownOpen && (
                <div className="absolute top-full right-0 mt-1 bg-white border border-gray-300 rounded shadow w-32 max-h-60 overflow-y-auto text-black z-50">
                  {LANGUAGE_OPTIONS.map((lang) => (
                    <div
                      key={lang.value}
                      onClick={() => {
